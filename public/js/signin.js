document.querySelectorAll("fieldset.with-placeholder input").forEach(function(el, idx) {
  ...
  // --- Existing mouth & arm animations remain ---

// 1. Blinking eyes
function blinkEyes() {
  let pupilRight = document.querySelector("#pupil-right");
  let pupilLeft = document.querySelector("#pupil-left");
  pupilRight.setAttribute("r", Math.random() > 0.5 ? 0.5 : 3);
  pupilLeft.setAttribute("r", Math.random() > 0.5 ? 0.5 : 3);
}
// Blink every 3 seconds
setInterval(blinkEyes, 3000);

// 2. Random floating movement (horizontal sway)
function randomFloat() {
  let ghost = document.querySelector("#ghost-body");
  let xOffset = (Math.random() - 0.5) * 5; // ±2.5 px
  ghost.style.transform = `translate(${xOffset}px, 0px)`;
}
// Float horizontally every 2 seconds
setInterval(randomFloat, 2000);

// 3. Mouth expressions based on email input (already exists) – you can add more moods
function updateMouthExpression() {
  let mouth = document.querySelector("#mouth");
  if (email.value.length === 0) {
    mouth.setAttribute("d", "M 75,115 C 79,120 91,126 101,125 110,125 126,118 127,114 125,117 117,125 101,125 85,126 79,117 75,115 Z"); // neutral
  } else if (email.value.includes("@")) {
    mouth.setAttribute("d", "M 75,115 C 79,110 92,117 102,117 111,117 123,111 127,114 131,117 123,136 102,136 81,137 73,121 75,115 Z"); // smile
  } else {
    mouth.setAttribute("d", "M 75,115 C 79,110 92,119 101,119 110,119 123,111 127,114 131,117 118,131 102,132 87,132 73,121 75,115 Z"); // confused
  }
}
email.addEventListener("input", updateMouthExpression);

// 4. Arm waving on hover of login button
let loginBtn = document.querySelector("input[type='submit']");
loginBtn.addEventListener("mouseenter", () => {
  document.querySelector("#ghost-arm-left").setAttribute("d","M 155,88 C 145,68 105,51 103,62 102,74 123,117 165,100");
});
